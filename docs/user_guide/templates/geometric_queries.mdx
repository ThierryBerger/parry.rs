...
{ /* TODO: explain coordinate system used can be local or global, for example  `QueryDispatcher::contact` returns local, but `query::contact` returns "absolute" */}
{ /* is it though ? I think we can just say absolute in conntext of `query::contact` ; and then a new paragraph pointing out dispatcher which returns local */ }

Here, _absolute coordinate system_ is the set of axes that are not relative
to any object. The last `dist` field requires some details. Sometimes, the
